...
const ChatPage = () => {
    ...
    const [replyingTo, setReplyingTo] = useState(null); // { id, message, sender_id }
    ...
    return (
        <div style={{ display: 'flex', height: 'calc(100vh - 160px)', gap: '24px' }}>
            ...
            {/* ── Sidebar ─────────────────────────────────────────────────── */}
            <div className="glass-effect" style={{ width: '320px', display: 'flex', flexDirection: 'column', background: 'var(--surface-1)', overflow: 'hidden' }}>
            ...
            {/* ── Chat Window ─────────────────────────────────────────────── */}
            <div className="glass-effect" style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--surface-1)', overflow: 'hidden' }}>

                {error && (
                    <div style={{ padding: '12px 24px', background: '#fee2e2', color: '#dc2626', fontSize: '0.85rem', fontWeight: 600, borderBottom: '1px solid #fca5a5' }}>
                        ⚠️ {error}
                    </div>
                )}
                ...
                {activeChat ? (
                    <>
                        ...
                        <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--glass-border)', display: 'flex', alignItems: 'center', gap: '12px' }}>
                            <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: 'var(--secondary)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800 }}>
                                {getPartnerInitial(activeChat)}
                            </div>
                            <div>
                                <h4 style={{ margin: 0, fontSize: '1rem' }}>{getPartnerName(activeChat)}</h4>
                        ...
                        <div style={{ flex: 1, overflowY: 'auto', padding: '24px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                            {messages.map((m) => {
                                ...
                                const tsMillis = m.timestamp?.toMillis?.() ?? (m.timestamp?.seconds ? m.timestamp.seconds * 1000 : null);
                                const isMyMessage = m.sender_id === (user.dbId || user.uid);
                                    ...
                                    >
                                        ...
                                        {parentMsg && (
                                            <div style={{
                                                ...
                                                <span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', display: 'inline-block', maxWidth: '220px', verticalAlign: 'bottom' }}>
                                                    {parentMsg.message}
                                                </span>
                                            </div>
                                        )}
                                        ...
                                        {/* Timestamp + status */}
                                        <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', marginTop: '4px', textAlign: isMe ? 'right' : 'left', fontWeight: 600 }}>
                                            {timeDisplay}
                        ...
                        <div style={{ borderTop: '1px solid var(--glass-border)' }}>
                            ...
                            <form onSubmit={handleSendMessage} style={{ padding: '20px 24px', display: 'flex', gap: '12px' }}>
                                <input
                                    ref={inputRef}
                                    type="text"
                                ...
                                <button
                                    ...
                                    type="submit"
                                    style={{ padding: '0 28px', borderRadius: '100px', opacity: sending ? 0.6 : 1 }}
